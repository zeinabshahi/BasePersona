...
export default function WalletDashboard({ address }: { address: string }) {
  ...
  const [notice, setNotice] = React.useState<'not_in_dataset' | null>(null)
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const demo = genDemoPayload(202307) as unknown as Payload
        demo.monthly.sort((a: any, b: any) => a.month.localeCompare(b.month))
        if (!dead) {
          setData(demo)
          const last = demo.monthly.length - 1
          setSel(last)
          const start = Math.max(0, demo.monthly.length - 12)
          setRange([start, last])
          setPreset('last12')
          setNotice('not_in_dataset')
  ...
  const selLocal = Math.min(Math.max(sel - r0, 0), Math.max(view.length - 1, 0))
  ...
  const dVolUsd = delta(
    cur?.volume_usd ?? (cur?.swap_volume_usd || 0) + (cur?.bridge_volume_usd || 0),
  ...
  return (
    <>
      {notice === 'not_in_dataset' && (
        ...
        >
          This address is not currently included in our “Top 1,000,000 Base wallets” dataset. Showing demo preview.
        </div>
      )}

      {/* Preset & current month */}
      <div style={{ display: 'flex', gap: 8, margin: '6px 0 10px', alignItems: 'center' }}>
      ...
      <div className={styles.grid}>
        <MetricCard
          ...
          deltaAbs={dBalUsd.diff}
          deltaPct={dBalUsd.pct}
          rank={cur?.ranks?.balance?.rank ?? null}
          pct={cur?.ranks?.balance?.pct ?? null}
          series={balUsd}
          selectedIndex={selLocal}
        />
        <MetricCard
          title="Balance (ETH)"
          unit="eth"
          ...
          deltaAbs={dBalEth.diff}
          deltaPct={dBalEth.pct}
          series={balEth}
          selectedIndex={selLocal}
        />
        <MetricCard
          ...
          deltaAbs={dVolUsd.diff}
          deltaPct={dVolUsd.pct}
          rank={cur?.ranks?.volume?.rank ?? null}
          pct={cur?.ranks?.volume?.pct ?? null}
          series={volUsd}
          selectedIndex={selLocal}
        />
        <MetricCard
          ...
          deltaAbs={dGasEth.diff}
          deltaPct={dGasEth.pct}
          series={gasEth}
          selectedIndex={selLocal}
        />
        <MetricCard
        ...
        />
        <MetricCard
          title="Interactions (txs)"
          unit={undefined}
          value={(cur?.swap_tx_count || 0) + (cur?.bridge_in_count || 0) + (cur?.bridge_out_count || 0)}
          subtitle="swap + bridge I/O"
          deltaAbs={dInter.diff}
          deltaPct={dInter.pct}
          rank={cur?.ranks?.activity?.rank ?? null}
          pct={cur?.ranks?.activity?.pct ?? null}
          series={inters}
          selectedIndex={selLocal}
        />
      </div>
