...
type Monthly = {
  ...
  uniq_weeks?: number          // streak (best streak days, within month)
...
function adapt(doc: WalletJson): Payload {
  ...
  const lifetime = (doc as any).lifetime ?? {}
  const summary: Summary = {
    ...
    // رتبهٔ تجمیعی (اختیاری)
    cum_ranks: {
      overall: {
        score: 0,
        rank: (doc as any).rank ?? 0,
        pct: 0,
      },
      balance: undefined,
      activity: undefined,
